package com.ryuqq.adapter.in.rest.fixture.response;

import com.ryuqq.adapter.in.rest.codingrule.dto.response.CodingRuleApiResponse;
import java.util.List;

/**
 * CodingRuleApiResponse Test Fixture
 *
 * @author development-team
 * @since 1.0.0
 */
public final class CodingRuleApiResponseFixture {

    private CodingRuleApiResponseFixture() {
        throw new AssertionError("Utility class - do not instantiate");
    }

    public static CodingRuleApiResponse valid() {
        return new CodingRuleApiResponse(
                1L,
                1L,
                null,
                "AGG-001",
                "Lombok 사용 금지",
                "BLOCKER",
                "ANNOTATION",
                "Domain Layer에서 Lombok 사용을 금지합니다",
                "Pure Java 원칙",
                false,
                List.of("AGGREGATE", "VALUE_OBJECT"),
                "2024-01-01T00:00:00Z",
                "2024-01-01T00:00:00Z");
    }
}
