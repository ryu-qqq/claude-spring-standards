...
/**
 * SearchRuleExamplesCursorApiRequest - RuleExample 복합 조건 조회 API Request (커서 기반)
 *
 * <p>RuleExample 복합 조건 조회 REST API 요청 DTO입니다. 코딩 규칙 ID, 예시 타입, 언어 필터링을 지원합니다.
 *
 ...
 * @since 1.0.0
 */
@Schema(description = "RuleExample 복합 조건 조회 요청 (커서 기반)")
public record SearchRuleExamplesCursorApiRequest(
        ...
        @Parameter(description = "코딩 규칙 ID 필터 (복수 선택 가능)", example = "1,2,3")
                ...
                List<Long> ruleIds,
        @Parameter(description = "예시 타입 필터 (복수 선택 가능)", example = "GOOD,BAD")
                @Schema(description = "예시 타입 필터 목록", nullable = true)
                List<String> exampleTypes,
        @Parameter(description = "언어 필터 (복수 선택 가능)", example = "JAVA,KOTLIN")
                @Schema(description = "언어 필터 목록", nullable = true)
