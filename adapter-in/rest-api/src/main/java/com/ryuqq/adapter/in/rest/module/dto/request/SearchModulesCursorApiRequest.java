...
/**
 * SearchModulesCursorApiRequest - Module 복합 조건 조회 API Request (커서 기반)
 *
 * <p>Module 복합 조건 조회 REST API 요청 DTO입니다. layerIds 필터링을 지원합니다.
 *
 ...
 * @since 1.0.0
 */
@Schema(description = "Module 복합 조건 조회 요청 (커서 기반)")
public record SearchModulesCursorApiRequest(
        ...
        @Parameter(description = "슬라이스 크기", example = "20")
                ...
                Integer size,
        @Parameter(description = "레이어 ID 필터 (복수 선택 가능)", example = "1,2,3")
                @Schema(description = "레이어 ID 필터 목록", nullable = true)
                List<Long> layerIds) {}
