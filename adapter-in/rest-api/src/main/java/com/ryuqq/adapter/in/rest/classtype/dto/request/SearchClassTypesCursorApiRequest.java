package com.ryuqq.adapter.in.rest.classtype.dto.request;
...
/**
 * SearchClassTypesCursorApiRequest - ClassType 복합 조건 조회 API Request (커서 기반)
 *
 * <p>ClassType 복합 조건 조회 REST API 요청 DTO입니다.
 *
 ...
 * @param categoryIds Category ID 필터 목록 (null 또는 빈 리스트면 전체 조회)
 * @param searchField 검색 필드 (CODE, NAME, DESCRIPTION, null이면 검색 안 함)
 * @param searchWord 검색어 (부분 일치, 최대 255자, null이면 검색 안 함)
 * @author ryu-qqq
 * @since 1.0.0
 */
@Schema(description = "ClassType 복합 조건 조회 요청 (커서 기반)")
public record SearchClassTypesCursorApiRequest(
        ...
        @Parameter(description = "슬라이스 크기", example = "20")
                ...
                Integer size,
        @Parameter(description = "Category ID 필터 (복수 선택 가능)", example = "1,2,3")
                @Schema(description = "Category ID 필터 목록", nullable = true)
        ...
        @Parameter(description = "검색 필드 (CODE, NAME, DESCRIPTION)", example = "CODE")
                @Schema(
                        ...
                        nullable = true)
                String searchField,
        @Parameter(description = "검색어 (부분 일치)", example = "AGGREGATE")
