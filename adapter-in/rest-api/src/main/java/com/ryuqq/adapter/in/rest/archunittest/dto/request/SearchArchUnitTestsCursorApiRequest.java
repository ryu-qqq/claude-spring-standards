...
/**
 * SearchArchUnitTestsCursorApiRequest - ArchUnitTest 복합 조건 조회 API Request (커서 기반)
 *
 * <p>ArchUnitTest 복합 조건 조회 REST API 요청 DTO입니다. 패키지 구조 ID(복수), 검색 필드/검색어, 심각도(복수) 필터링을 지원합니다.
 *
 ...
 * @param searchField 검색 필드 (CODE, NAME, DESCRIPTION, TEST_CLASS_NAME, TEST_METHOD_NAME)
 * @param searchWord 검색어
 * @param severities 심각도 필터 목록 (null 또는 빈 리스트면 전체 조회)
 * @author ryu-qqq
 * @since 1.0.0
 */
@Schema(description = "ArchUnitTest 복합 조건 조회 요청 (커서 기반)")
public record SearchArchUnitTestsCursorApiRequest(
        ...
        @Parameter(description = "검색 필드", example = "CODE")
                @Schema(
                        description =
                                "검색 필드 (CODE, NAME, DESCRIPTION, TEST_CLASS_NAME,"
                                        + " TEST_METHOD_NAME)",
                        nullable = true)
                String searchField,
        @Parameter(description = "검색어", example = "AGG-001")
                @Schema(description = "검색어", nullable = true)
                String searchWord,
        @Parameter(description = "심각도 필터 (복수 선택 가능)", example = "CRITICAL,BLOCKER")
