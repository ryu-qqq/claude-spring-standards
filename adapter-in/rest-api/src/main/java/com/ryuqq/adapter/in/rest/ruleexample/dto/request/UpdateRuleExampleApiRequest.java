...
/**
 * UpdateRuleExampleApiRequest - RuleExample 수정 API Request
 *
 * <p>RuleExample 수정 REST API 요청 DTO입니다.
 *
 ...
 * @param explanation 설명 (필수)
 * @param highlightLines 하이라이트 라인 목록 (필수)
 * @author ryu-qqq
 * @since 1.0.0
 */
@Schema(description = "규칙 예시 수정 요청")
public record UpdateRuleExampleApiRequest(
        ...
        @Schema(description = "언어", example = "JAVA")
                ...
                String language,
        @Schema(description = "설명", example = "올바른 서비스 클래스 정의 방법")
                @NotNull(message = "explanation은 필수입니다")
                @Size(max = 2000, message = "explanation은 2000자 이내여야 합니다")
                String explanation,
        @Schema(description = "하이라이트 라인 목록", example = "[1, 3, 5]")
                @NotNull(message = "highlightLines는 필수입니다")
