...
/**
 * SearchResourceTemplatesCursorApiRequest - ResourceTemplate 복합 조건 조회 API Request (커서 기반)
 *
 * <p>ResourceTemplate 복합 조건 조회 REST API 요청 DTO입니다. 모듈 ID(복수), 카테고리(복수), 파일 타입(복수) 필터링을 지원합니다.
 *
 ...
 * @since 1.0.0
 */
@Schema(description = "ResourceTemplate 복합 조건 조회 요청 (커서 기반)")
public record SearchResourceTemplatesCursorApiRequest(
        ...
        @Parameter(description = "슬라이스 크기", example = "20")
                ...
                Integer size,
        @Parameter(description = "모듈 ID 필터 (복수 선택 가능)", example = "1,2,3")
                @Schema(description = "모듈 ID 필터 목록", nullable = true)
                List<Long> moduleIds,
        @Parameter(description = "카테고리 필터 (복수 선택 가능)", example = "CONFIG,I18N")
                @Schema(description = "카테고리 필터 목록", nullable = true)
                List<String> categories,
        @Parameter(description = "파일 타입 필터 (복수 선택 가능)", example = "YAML,JSON")
                @Schema(description = "파일 타입 필터 목록", nullable = true)
