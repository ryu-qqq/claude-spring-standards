...
/**
 * TechStackQueryController 통합 테스트
 *
 * <p>검증 범위:
 *
 * <ul>
...
@ActiveProfiles("test")
@DisplayName("TechStackQueryController 통합 테스트")
class TechStackQueryControllerTest {
    ...
    class GetAllTechStacks {
        ...
        void validRequest_ShouldReturn200() {
            // Given
            String url = TechStackApiEndpoints.TECH_STACKS + "?size=20";

            // When
            ResponseEntity<ApiResponse<SliceApiResponse<TechStackApiResponse>>> response =
                    restTemplate.exchange(
                            url,
                            HttpMethod.GET,
                            null,
                            new ParameterizedTypeReference<
                                    ApiResponse<SliceApiResponse<TechStackApiResponse>>>() {});
        ...
        void sizeOutOfRange_ShouldReturn400() {
            ...
            ResponseEntity<ApiResponse<SliceApiResponse<TechStackApiResponse>>> response =
                    restTemplate.exchange(
                            TechStackApiEndpoints.TECH_STACKS + "?size=101",
                            HttpMethod.GET,
                            null,
                            new ParameterizedTypeReference<
                                    ApiResponse<SliceApiResponse<TechStackApiResponse>>>() {});
