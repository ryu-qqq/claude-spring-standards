package com.ryuqq.domain.layer.query;
...
/**
 * LayerSliceCriteria - Layer 슬라이스 조회 조건 (커서 기반)
 *
 * <p>Layer 목록 조회 시 사용되는 커서 기반 페이징 조건입니다.
 *
...
public record LayerSliceCriteria(
     ...
     */
    public static LayerSliceCriteria first(int size) {
        return new LayerSliceCriteria(null, null, null, CursorPageRequest.first(size));
    }
     ...
     * @return LayerSliceCriteria
     */
    public static LayerSliceCriteria firstByArchitecture(ArchitectureId architectureId, int size) {
        return new LayerSliceCriteria(
                List.of(architectureId), null, null, CursorPageRequest.first(size));
    }
     ...
     * @return LayerSliceCriteria
     */
    public static LayerSliceCriteria afterId(Long cursorId, int size) {
        return new LayerSliceCriteria(null, null, null, CursorPageRequest.afterId(cursorId, size));
    }

    /**
     * LayerSliceCriteria 생성 (static factory method)
     *
     * @param cursorPageRequest 커서 기반 페이징 요청
     * @param architectureIds Architecture ID 목록
     * @param searchField 검색 필드
     * @param searchWord 검색어
     * @return LayerSliceCriteria 인스턴스
     */
    public static LayerSliceCriteria of(
            CursorPageRequest<Long> cursorPageRequest,
            List<ArchitectureId> architectureIds,
            String searchField,
            String searchWord) {
        return new LayerSliceCriteria(architectureIds, searchField, searchWord, cursorPageRequest);
    }
