...
class ExceptionArchTest {
    ...
    /**
     * 규칙 8: ErrorCode Enum은 getHttpStatus() 메서드 반환 타입 검증
     *
     * <p>Note: Domain 레이어는 Spring HttpStatus에 직접 의존하지 않습니다. getHttpStatus()는 int 또는 커스텀 타입을 반환하도록
     * 설계되어야 합니다.
     */
    @Test
    ...
    /** getHttpStatus() 메서드가 적절한 반환 타입을 가지는지 검증 (int 또는 Spring HttpStatus를 사용하지 않는 커스텀 타입) */
                        ...
                        javaClass.getAllMethods().stream()
                                ...
                                .anyMatch(
                                        method -> {
                                            ...
                                            // int 타입이거나 org.springframework가 아닌 타입이어야 함
