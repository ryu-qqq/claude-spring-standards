...
2. If on main/master:
   ...
   - Create branch: `git checkout -b feature/{ISSUE-KEY}-persistence`
   - Example: `feature/MEMBER-001-persistence`
...
1. **Check Git Branch** (see MUST DO section above):
   ...
   - Extract Issue Key from plan file (e.g., MEMBER-001)
   - If feature/{ISSUE-KEY}-persistence branch doesn't exist, create it
   - Checkout to feature branch before starting TDD

2. **Read plan file** from `docs/prd/plans/{ISSUE-KEY}-persistence-plan.md`

3. **Find the next unmarked test** in the Persistence Layer section
