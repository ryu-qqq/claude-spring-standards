...
### 6. Queue 자동 관리 시스템 (100% 자동화) ⭐ NEW v2.5

**위치**: `.claude/queue/`, `.claude/hooks/user-prompt-submit.sh`, `.claude/hooks/after-tool-use.sh`
...
사용자 프롬프트
    ↓
user-prompt-submit.sh (Hook 실행)
    ├─ 키워드 분석: "domain", "usecase", "controller" 등
    ├─ Context Score 계산 (0-100+)
    ├─ Score >= 25? → Queue 자동 추가
    └─ queue-manager.sh add/start
         ↓
Claude Code (작업 수행)
    ├─ TodoWrite 동기화
    └─ Write/Edit 도구 사용
...
**자동 작업 추가**:
- Context Score >= 25이면 자동으로 Queue에 추가
- Hook JSON 필터링 (실제 프롬프트만 추출)
- Layer 정보 자동 태깅 (domain, application, persistence, adapter-rest)

**자동 작업 완료**:
- Write/Edit 도구 사용 시 자동 완료
- Duration 자동 계산 및 기록
- completed.jsonl에 이력 저장

**TodoWrite 통합**:
- TodoWrite `in_progress` → Queue add + start
- TodoWrite `completed` → Queue complete
- 양방향 동기화 (sync-todo-to-queue.py)

**Statusline 통합**:
- 실시간 Queue 상태 표시 (`📋 N`)
- context-monitor.py에 통합
- 자동 업데이트 (Hook 이벤트 기반)
...
**자동 모드** (기본):
```bash
# 1. Context Score >= 25인 프롬프트 입력
"Order Domain Aggregate를 생성해줘"

# 2. 자동 실행:
#    - Hook이 키워드 감지 ("domain", "aggregate")
#    - Queue 자동 추가 및 시작
#    - 작업 수행
#    - Write/Edit 시 자동 완료

# 3. Statusline에서 실시간 확인
[Claude Sonnet 4.5] 📋 1 | 🧠 🟢████████ 38% | ...
```
...
# Queue 목록 확인
bash .claude/queue/queue-manager.sh list

# 작업 이력 확인 (최근 10개)
bash .claude/queue/queue-manager.sh history 10

# Queue 초기화
bash .claude/queue/queue-manager.sh clear
```

#### 성능 지표

- **자동화율**: 100% (수동 작업 0회)
- **추가 지연**: <50ms (Hook 실행 시간)
- **완료 지연**: <30ms (Hook 실행 시간)
- **메모리 사용**: 최소 (JSONL 기반)
- **확장성**: 무제한 작업 이력 (completed.jsonl)

#### 통합 가이드

자세한 통합 방법은 다음 문서 참조:
- `.claude/queue/README.md` - 사용자 가이드
- `.claude/queue/INTEGRATION_GUIDE.md` - 기술 통합 가이드
