 ...
 *   <li>UseCase 메서드 네이밍 (execute/query 접두사 필수)</li>
 ...
 * @author Claude Code
 * @since 2025-11-04
 * @see com.ryuqq.bootstrap.architecture.DomainLayerRulesTest
 * @see com.ryuqq.bootstrap.architecture.ApplicationLayerRulesTest
 * @see com.ryuqq.bootstrap.architecture.JpaEntityConventionTest
 */
...
class ZeroToleranceArchitectureTest {
    ...
    class LombokProhibitionTest {
        ...
        void shouldNotUseLombokData() {
            ArchRule rule = noClasses()
                .that().resideInAPackage("com.ryuqq..")
    ...
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    // 4️⃣ UseCase 메서드 네이밍 규칙 (Zero-Tolerance)
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

    @Nested
    @DisplayName("UseCase 메서드 네이밍 규칙")
    class UseCaseMethodNamingTest {

        @Test
        @DisplayName("Command UseCase 메서드는 execute로 시작해야 함")
        void commandUseCaseMethodsShouldStartWithExecute() {
            ArchCondition<JavaMethod> startsWithExecute = new ArchCondition<JavaMethod>("start with 'execute'") {
                @Override
                public void check(JavaMethod method, ConditionEvents events) {
                    // Command DTO를 파라미터로 받는 메서드만 검증
                    boolean hasCommandParameter = method.getRawParameterTypes().stream()
                        .anyMatch(param -> param.getSimpleName().endsWith("Command"));

                    if (hasCommandParameter && !method.getName().startsWith("execute")) {
                        String message = String.format(
                            "Command UseCase 메서드 %s는 'execute'로 시작해야 합니다. " +
                            "패턴: execute{Aggregate}{Action}(). 예: executeOrderCreation()",
                            method.getFullName()
                        );
            ...
            ArchRule rule = methods()
                ...
                .and().arePublic()
                .should(startsWithExecute)
                .because("Command UseCase 메서드는 execute{Aggregate}{Action}() 패턴을 따라야 합니다");

            rule.check(allClasses);
        }

        @Test
        @DisplayName("Query UseCase 메서드는 query로 시작해야 함")
        void queryUseCaseMethodsShouldStartWithQuery() {
            ArchCondition<JavaMethod> startsWithQuery = new ArchCondition<JavaMethod>("start with 'query'") {
                @Override
                public void check(JavaMethod method, ConditionEvents events) {
                    // Query DTO를 파라미터로 받는 메서드만 검증
                    boolean hasQueryParameter = method.getRawParameterTypes().stream()
                        .anyMatch(param -> param.getSimpleName().endsWith("Query"));

                    if (hasQueryParameter && !method.getName().startsWith("query")) {
                        String message = String.format(
                            "Query UseCase 메서드 %s는 'query'로 시작해야 합니다. " +
                            "패턴: query{Aggregate}By{Condition}() 또는 query{Aggregate}(). " +
                            "예: queryOrderById(), queryOrders()",
                            method.getFullName()
                        );
                        events.add(SimpleConditionEvent.violated(method, message));
                    }
            ...
            ArchRule rule = methods()
                .that().areDeclaredInClassesThat().resideInAPackage("..application..port.in..")
                .and().arePublic()
                .should(startsWithQuery)
                .because("Query UseCase 메서드는 query{Aggregate}By{Condition}() 패턴을 따라야 합니다");

            rule.check(allClasses);
        }

        @Test
        @DisplayName("UseCase 메서드는 execute 또는 query로만 시작해야 함")
        void useCaseMethodsShouldOnlyStartWithExecuteOrQuery() {
            ArchCondition<JavaMethod> startsCorrectly = new ArchCondition<JavaMethod>("start with 'execute' or 'query'") {
                @Override
                public void check(JavaMethod method, ConditionEvents events) {
                    String methodName = method.getName();

                    // java.lang.Object 메서드 제외
                    if (methodName.equals("equals") || methodName.equals("hashCode") ||
                        methodName.equals("toString") || methodName.equals("clone")) {
                        return;
                    }

                    if (!methodName.startsWith("execute") && !methodName.startsWith("query")) {
                        String message = String.format(
                            "UseCase 메서드 %s는 'execute' 또는 'query'로 시작해야 합니다. " +
                            "Command는 execute{Aggregate}{Action}(), Query는 query{Aggregate}By{Condition}() 패턴",
                            method.getFullName()
                        );
                        events.add(SimpleConditionEvent.violated(method, message));
                    }
            ...
            ArchRule rule = methods()
                ...
                .should(startsCorrectly)
                .because("UseCase 메서드는 CQRS 구분을 위해 execute 또는 query 접두사를 사용해야 합니다");
    ...
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    // 5️⃣ Orchestration Pattern (Zero-Tolerance)
    // ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
                ...
                .should().haveRawReturnType(new com.tngtech.archunit.base.DescribedPredicate<com.tngtech.archunit.core.domain.JavaClass>("Outcome") {
                    ...
                    public boolean test(com.tngtech.archunit.core.domain.JavaClass javaClass) {
                        return javaClass.getSimpleName().equals("Outcome") ||
                               javaClass.isAssignableTo("com.ryuqq.application.common.orchestration.Outcome");
                    }
                })
