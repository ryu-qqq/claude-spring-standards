...
> **⚠️ Zero-Tolerance**: `Search*UseCase`는 반드시 `*PageResponse` 또는 `*SliceResponse`를 반환해야 합니다. `List<T>` 반환은 ArchUnit 테스트 실패의 원인이 됩니다.
...
- [ ] Response DTO: `dto.response.{Bc}DetailResponse` 또는 `{Bc}PageResponse`
...
- [ ] 반환 타입: `*PageResponse` 또는 `*SliceResponse` (List 반환 금지)
- [ ] PageResponse 필드: `content`, `page`, `size`, `totalElements`, `totalPages`, `first`, `last`
- [ ] SliceResponse 필드: `content`, `size`, `hasNext`
...
**최종 수정일**: 2026-01-05
**버전**: 2.2.0 (Search UseCase 반환타입 규칙 Zero-Tolerance 추가)
