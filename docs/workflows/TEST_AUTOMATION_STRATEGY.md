# 테스트 자동화 전략 - Claude Code vs Cursor IDE

## 🎯 핵심 전략

**Cursor IDE**: 빠른 Test Fixture + Testcontainers 설정
...
```
       ...
       /------\
      / 통합   \      ← Claude Code + Cursor IDE (50:50)
     /----------\
...
- **담당**: **Claude Code (70%) + Cursor IDE (30%)**
- **Claude Code 역할**:
  - 복잡한 테스트 시나리오 작성
...
## 🔄 Cursor IDE Workflows 보강

### 1. `/test-integration-quick` ⭐ NEW

**목적**: 빠른 통합 테스트 템플릿 생성 (Cursor IDE 전용)

**Workflow 파일**: `.cursor/workflows/test-integration-quick.md`
...
**목적**: E2E 시나리오 템플릿 생성 (Cursor IDE → Claude 협업)

**Workflow 파일**: `.cursor/workflows/test-e2e-scenario.md`
...
**역할**: Cursor IDE는 기본 구조만, 상세 시나리오는 Claude Code
...
| 테스트 유형 | Cursor IDE | Claude Code | 비율 |
|------------|-----------|-------------|------|
| **단위 테스트** | ❌ | ✅ 전담 | 0:100 |
...
```bash
# 1️⃣ Cursor IDE: TestFixture + 기본 구조
...
1. **Cursor IDE**: 빠른 템플릿 생성 (Boilerplate)
