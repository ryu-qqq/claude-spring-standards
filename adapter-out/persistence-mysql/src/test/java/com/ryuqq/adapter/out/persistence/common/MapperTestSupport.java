 ...
 * <p><strong>특징:</strong>
 * <ul>
 *   <li>Spring Context 불필요 - 가장 빠른 실행</li>
 *   <li>변환 로직 검증에 집중</li>
 *   <li>리플렉션 기반 필드 비교 유틸리티 제공</li>
 * </ul>
 *
 * <h2>사용 예시:</h2>
 * <pre>{@code
...
public abstract class MapperTestSupport {
    ...
    protected void assertFieldsMatch(Object source, Object target, String... fieldNames) {
        for (String fieldName : fieldNames) {
            ...
            assertThat(targetValue)
                    .as("Field '%s' should match. Source: %s, Target: %s",
    ...
    protected void assertFieldMatch(
            Object source, String sourceField,
            Object target, String targetField) {

        Object sourceValue = getFieldValue(source, sourceField);
        Object targetValue = getFieldValue(target, targetField);

        assertThat(targetValue)
                .as("Field mapping '%s' -> '%s' should match. Source: %s, Target: %s",
    ...
    protected void assertAllFieldsNotNull(Object object, String... fieldNames) {
        ...
        for (String fieldName : fieldNames) {
            ...
            assertThat(fieldValue)
                    .as("Field '%s' should not be null", fieldName)
                    .isNotNull();
        }
    }
    ...
    protected void assertNullConversion(Object result, String inputDescription) {
        assertThat(result)
                .as("Conversion of %s should return null", inputDescription)
                .isNull();
    }
     ...
     * 양방향 변환 동등성 검증
     *
     * <p>Domain -> Entity -> Domain 또는 Entity -> Domain -> Entity
     * 변환 후 동등성을 검증합니다.
     *
    ...
    protected void assertRoundTripEquality(
        ...
        for (String fieldName : fieldNames) {
            ...
            assertThat(convertedValue)
                    .as("Round-trip conversion should preserve field '%s'. Original: %s, Converted: %s",
    ...
    protected void assertCollectionSizeMatch(
        ...
        assertThat(targetSize)
                .as("Collection '%s' size should match. Source: %d, Target: %d",
    ...
    protected Object extractVoValue(Object vo) {
        ...
        try {
            ...
            throw new RuntimeException("Failed to extract VO value from " + vo.getClass().getName(), e);
        }
    }
