...
class MapperArchTest {
    ...
    class MethodRules {
        ...
        /** 규칙 3-3: toEntity() 메서드 권장 (경고) */
        @Test
        @DisplayName("규칙 3-3: toEntity() 메서드를 권장합니다 (경고)")
        void mapper_ShouldHaveToEntityMethod_Warning() {
            // ObjectMapper 설정 클래스 등 특수한 Mapper는 제외하고 경고만 출력
            mapperClasses.stream()
                    .filter(javaClass -> javaClass.getSimpleName().endsWith("Mapper"))
                    .filter(javaClass -> !javaClass.getSimpleName().endsWith("ObjectMapper"))
                    .forEach(
                            javaClass -> {
                                ...
                                if (!hasToEntityMethod) {
                                    System.err.println(
                                            "⚠️ [WARNING] "
                                                    ...
                                                    + "에 public toEntity() 메서드가 권장됩니다 (Domain →"
                                                    + " Entity 변환)");
                                }
                            });
        }

        /** 규칙 3-4: toDomain() 메서드 권장 (경고) */
        @Test
        @DisplayName("규칙 3-4: toDomain() 메서드를 권장합니다 (경고)")
        void mapper_ShouldHaveToDomainMethod_Warning() {
            // ObjectMapper 설정 클래스 등 특수한 Mapper는 제외하고 경고만 출력
            mapperClasses.stream()
                    .filter(javaClass -> javaClass.getSimpleName().endsWith("Mapper"))
                    .filter(javaClass -> !javaClass.getSimpleName().endsWith("ObjectMapper"))
                            ...
                            javaClass -> {
                                ...
                                if (!hasToDomainMethod) {
                                    System.err.println(
                                            "⚠️ [WARNING] "
                                                    + javaClass.getSimpleName()
                                                    + "에 public toDomain() 메서드가 권장됩니다 (Entity →"
                                                    + " Domain 변환)");
                                }
                            });
        }
