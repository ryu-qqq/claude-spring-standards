 ...
 * <ul>
 *   <li>일반 Query 전용 (5개 메서드: findById, findByIdIncludingDeleted, existsById, findByCriteria, countByCriteria)</li>
...
class QueryDslRepositoryArchTest {
    ...
    class MethodRules {
        ...
        @Test
        @DisplayName("규칙 2-1: 5개 표준 메서드 권장 (경고)")
        void queryDslRepository_ShouldHaveOnlyStandardMethods_Warning() {
            var standardMethods = java.util.Set.of(
                "findById", "findByIdIncludingDeleted", "existsById",
                "findByCriteria", "countByCriteria",
                "search", "count" // search와 count도 findByCriteria/countByCriteria와 동등
            );

            queryDslRepositoryClasses.stream()
                .filter(javaClass -> javaClass.getSimpleName().endsWith("QueryDslRepository"))
                .forEach(javaClass -> {
                    var nonStandardMethods = javaClass.getMethods().stream()
                        ...
                        .filter(method -> !standardMethods.contains(method.getName()))
                    ...
                    if (!nonStandardMethods.isEmpty()) {
                        System.err.println(
                            "⚠️ [WARNING] " + javaClass.getSimpleName()
                                + "에 비표준 메서드가 있습니다: " + nonStandardMethods
                                + ". 표준 메서드(findById, existsById, findByCriteria, countByCriteria) 사용을 권장합니다.");
                    }
                });
            // 경고만 출력하고 테스트는 통과
        }
