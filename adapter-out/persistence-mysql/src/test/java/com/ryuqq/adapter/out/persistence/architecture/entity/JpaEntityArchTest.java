 ...
 *   <li>JPA 전용 Enum 금지 (2개 규칙) - Domain VO 사용 필수
 *   <li>메서드 패턴 규칙 (2개 규칙)
...
class JpaEntityArchTest {
    ...
    // ===== 2.5 JPA 전용 Enum 금지 규칙 =====

    @Nested
    @DisplayName("JPA 전용 Enum 금지 규칙 (Domain VO 사용)")
    class JpaEnumProhibitionRules {

        @Test
        @DisplayName("*Jpa Enum 클래스 존재 금지 (Domain VO 사용 필수)")
        void persistence_MustNotHaveJpaSpecificEnums() {
            ArchRule rule =
                    classes()
                            .that()
                            .areEnums()
                            .and()
                            .haveSimpleNameEndingWith("Jpa")
                            .should()
                            .notBeEnums()
                            .allowEmptyShould(true)
                            .because(
                                    "Persistence Layer에 JPA 전용 Enum 정의가 금지됩니다. "
                                            + "Domain Layer의 VO(Enum)를 직접 사용하세요");

            rule.check(allClasses);
        }

        @Test
        @DisplayName("entity 패키지 내 Enum 클래스 존재 금지")
        void entityPackage_MustNotContainEnums() {
            ArchRule rule =
                    classes()
                            .that()
                            .areEnums()
        ...
        void jpaEntity_MustBeAnnotatedWithEntity() {
            ArchRule rule =
                    classes()
                            ...
                            .and()
                            .haveSimpleNameNotStartingWith("Q") // QueryDSL Q-class 제외
                            .should()
