package com.ryuqq.application.convention.dto.query;

import com.ryuqq.application.common.dto.query.CommonCursorParams;
import java.util.List;

/**
 * ConventionSearchParams - Convention 목록 조회 SearchParams DTO
 *
 * <p>Convention 목록을 커서 기반으로 조회하는 SearchParams DTO입니다. 모듈 ID 필터링을 지원합니다.
 *
 ...
 * @since 1.0.0
 */
public record ConventionSearchParams(CommonCursorParams cursorParams, List<Long> moduleIds) {

    /**
     * ConventionSearchParams 생성 (페이징만)
     *
     * @param cursorParams 커서 기반 페이징 파라미터
     * @return ConventionSearchParams 인스턴스
     */
    public static ConventionSearchParams of(CommonCursorParams cursorParams) {
        return new ConventionSearchParams(cursorParams, null);
    }

    /**
     * ConventionSearchParams 생성 (필터 포함)
     *
     * @param cursorParams 커서 기반 페이징 파라미터
     * @param moduleIds 모듈 ID 필터 목록
     * @return ConventionSearchParams 인스턴스
     */
    public static ConventionSearchParams of(CommonCursorParams cursorParams, List<Long> moduleIds) {
        return new ConventionSearchParams(cursorParams, moduleIds);
    }
