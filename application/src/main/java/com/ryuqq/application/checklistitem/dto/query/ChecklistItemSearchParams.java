package com.ryuqq.application.checklistitem.dto.query;

import com.ryuqq.application.common.dto.query.CommonCursorParams;
import java.util.List;

/**
 * ChecklistItemSearchParams - ChecklistItem 목록 조회 SearchParams DTO
 *
 * <p>ChecklistItem 목록을 커서 기반으로 조회하는 SearchParams DTO입니다. 코딩 규칙 ID, 체크 타입, 자동화 도구 필터링을 지원합니다.
 *
 ...
 * @since 1.0.0
 */
public record ChecklistItemSearchParams(
    ...
    /**
     * ChecklistItemSearchParams 생성 (페이징만)
     *
     * @param cursorParams 커서 기반 페이징 파라미터
     * @return ChecklistItemSearchParams 인스턴스
     */
    public static ChecklistItemSearchParams of(CommonCursorParams cursorParams) {
        return new ChecklistItemSearchParams(cursorParams, null, null, null, null);
    }

    /**
     * ChecklistItemSearchParams 생성 (필터 포함)
     *
     * @param cursorParams 커서 기반 페이징 파라미터
     * @param ruleIds 코딩 규칙 ID 필터 목록
     * @param checkTypes 체크 타입 필터 목록
     * @param automationTools 자동화 도구 필터 목록
     * @param isCritical 필수 여부 필터
     * @return ChecklistItemSearchParams 인스턴스
     */
    public static ChecklistItemSearchParams of(
            CommonCursorParams cursorParams,
            List<Long> ruleIds,
            List<String> checkTypes,
            List<String> automationTools,
            Boolean isCritical) {
        return new ChecklistItemSearchParams(
                cursorParams, ruleIds, checkTypes, automationTools, isCritical);
    }
