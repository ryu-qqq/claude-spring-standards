...
/**
 * SearchArchUnitTestsByCursorUseCase - ArchUnitTest 복합 조건 조회 UseCase (커서 기반)
 *
 * <p>ArchUnitTest 목록을 커서 기반으로 복합 조건(패키지 구조 ID, 검색 필드/검색어, 심각도)으로 조회합니다.
 *
 ...
 * @author ryu-qqq
 */
public interface SearchArchUnitTestsByCursorUseCase {

    /**
     * ArchUnitTest 복합 조건 조회 실행 (커서 기반)
     *
     * @param searchParams 조회 SearchParams DTO
     * @return ArchUnitTest 슬라이스 결과
     */
    ArchUnitTestSliceResult execute(ArchUnitTestSearchParams searchParams);
}
