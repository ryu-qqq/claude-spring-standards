package com.ryuqq.application.module.dto.query;

import com.ryuqq.application.common.dto.query.CommonCursorParams;
import java.util.List;

/**
 * ModuleSearchParams - Module 목록 조회 SearchParams DTO
 *
 * <p>Module 목록을 커서 기반으로 조회하는 SearchParams DTO입니다. layerIds 필터링을 지원합니다.
 *
 ...
 * @param layerIds 레이어 ID 필터 목록 (null 또는 빈 리스트면 전체 조회)
 * @author ryu-qqq
 * @since 1.0.0
 */
public record ModuleSearchParams(CommonCursorParams cursorParams, List<Long> layerIds) {

    /**
     * ModuleSearchParams 생성 (페이징만)
     *
     * @param cursorParams 커서 기반 페이징 파라미터
     * @return ModuleSearchParams 인스턴스
     */
    public static ModuleSearchParams of(CommonCursorParams cursorParams) {
        return new ModuleSearchParams(cursorParams, null);
    }

    /**
     * ModuleSearchParams 생성 (필터 포함)
     *
     * @param cursorParams 커서 기반 페이징 파라미터
     * @param layerIds 레이어 ID 필터 목록
     * @return ModuleSearchParams 인스턴스
     */
    public static ModuleSearchParams of(CommonCursorParams cursorParams, List<Long> layerIds) {
        return new ModuleSearchParams(cursorParams, layerIds);
    }
